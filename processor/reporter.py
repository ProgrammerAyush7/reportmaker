...
def generate_report(input_json, db_path="processor/info.db"):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    cursor.execute("""
        SELECT sr_no, name, normal, unit, additional, category
        FROM tests_new
    """)
    rows = cursor.fetchall()

    # Map test code -> test row
    tests = {str(r[0]): r for r in rows}

    report = {}

    for code, value in input_json["tests"].items():

        if code not in tests:
            continue

        rowid, name, normal, unit, additional, category = tests[code]

        if category not in report:
            report[category] = []

        # -------------------------------------------------
        # URINE (keep full reading as one block)
        # -------------------------------------------------
        if category.strip().lower() == "urine":
        ...
        # NO SUBTESTS
        # -------------------------------------------------
        elif additional is None or additional.strip() == "":
            report[category].append({
                "test": name,
                "reading": value,
                "unit": unit,
        ...
        # -------------------------------------------------
            ...
            subtests = [x.strip() for x in additional.split(",")]
            normals = [x.strip() for x in normal.split(",")] if normal else []
            readings = [x.strip() for x in value.split(",")]

            for i, sub in enumerate(subtests):
                reading = readings[i] if i < len(readings) else None
                normal_range = normals[i] if i < len(normals) else None
...
#         "81":"12",
#         "82":"12",
#         "82":"Random,Turbid,10 ml.,Pale Yellow,1.019,as+,NIL,NIL,---,---,3-4 /HPF,--- /HPF,5-6 /HPF,---,---,---,---,---"
#     }
# }
...
#   "sex": "Male",
#   "date": "05-03-26",
#   "referred_by": "Dr. Imran Husain Sir",
# }

# report_json = generate_report(input_data)
# print(report_json)
# generate_pdf_report(report_json, patient_data, "processor/templatenew.png", "report.pdf")
